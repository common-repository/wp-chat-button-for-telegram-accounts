jQuery(document).ready(function () {
  ...
  const install_failed = `<div class="fbv-noti-install-failed"><div class="fbv-label-error">Oops! Installation failed.</div><div>Please try <a href="${rjteCross.filebird_install_url}">manual installation</a>.</div></div>`
  ...
  jQuery('body.upload-php #wpfooter').exists(function () {
    rjteCross.show_popup && this.append(fbv_cross)
  })
  ...
  jQuery('.fbv-cross-link.fbv-cross-hide-popup').click(function () {
    ...
    jQuery.ajax({
      ...
      method: 'POST',
      data: {
        action: 'rjte_filebird_cross_hide',
        nonce: rjteCross.nonce,
        type: 'popup'
      },
      beforeSend: function(){
  ...
  jQuery('.fbv-cross-link.fbv-cross-hide-notification').click(function(){
    jQuery.ajax({
      ...
      if (result.success) {
        jQuery('#rjte-ads-wrapper button.notice-dismiss').click()
      } else {
  ...
  jQuery('.fbv-cross-install:not(.fbv_installing)').click(function (e) {
    ...
    jQuery.ajax({
      ...
      method: 'POST',
      data: {
          action: 'rjte_filebird_cross_install',
          nonce: rjteCross.nonce
      },
      beforeSend: function(){
        a.focusout()
      ...
      success: function(response){
        if (response.success) {
          ...
          a.html(done)
          a.off('click')
          a.click(()=> { window.location.href = rjteCross.media_url })
        } else {
