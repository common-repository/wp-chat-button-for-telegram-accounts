    ...
    class rjteCross
    {
        ...
        public static function get_instance($pluginPrefix, $pluginInstallSearching, $pluginDirURL, $pluginFolderSlug)
        {
            if (null == self::$instance) {
                self::$instance = new self($pluginPrefix, $pluginInstallSearching, $pluginDirURL, $pluginFolderSlug);
        ...
        public function doHooks() {
            add_action('init', function(){
                if (!$this->is_plugin_exist()) {
                    $notificationOption = get_option("rjte_notification_{$this->pluginPrefix}_cross"); //Save the next time notification will appear
                    $popupOption        = get_option("rjte_popup_{$this->pluginPrefix}_cross"); //Save the next time notification will appear
         
                    if ($notificationOption === false || time() >= $notificationOption) {
                        add_action('admin_notices', array($this, 'add_notification'));
                        add_action("wp_ajax_rjte_{$this->pluginPrefix}_cross_notification", array($this, 'ajax_set_notification'));
                    }
                    ...
                    add_action("wp_ajax_rjte_{$this->pluginPrefix}_cross_install", array($this, 'ajax_install_plugin'));
                    add_action("wp_ajax_rjte_{$this->pluginPrefix}_cross_hide", array($this, 'ajax_hide_cross'));
                }
            });
        }
        ...
        public function need_update_option(){ 
            ...
            update_option("rjte_popup_{$this->pluginPrefix}_cross", $time);
            update_option("rjte_notification_{$this->pluginPrefix}_cross", $time);
        }
        ...
        public function add_global_script_styles()
        {
            ...
            wp_register_script("rjte-popup-{$this->pluginPrefix}-cross", $this->pluginDirURL . 'assets/js/cross.js', ['jquery'], '1.0', true);
            wp_localize_script("rjte-popup-{$this->pluginPrefix}-cross", 'rjteCross', array(
                'nonce' => wp_create_nonce("rjte_{$this->pluginPrefix}_cross_nonce"),
                'media_url' => admin_url('upload.php'),
                'filebird_install_url' => $url,
                'show_popup' => $this->showPopup
            ));
            wp_enqueue_script("rjte-popup-{$this->pluginPrefix}-cross");
            ?>
            <style>
        ...
        public function add_notification() {
            ...
            ?>
            <div class="notice notice-info is-dismissible" id="rjte-ads-wrapper">
                <div class="rjte-d-row rjte-justify-between">
                    <div class="rjte-ads-info">
                    <h4 class="rjte-ads-title"><?php _e('Recommend', 'rj-telegram') ?></h4>
                    <p>To easily manage your files in WordPress media library with folders, please try FileBird plugin.</p>
                    <div class="rjte-btn-row">
                    ...
                    <img class="rjte-ads-img" src="<?php echo RJ_TELEGRAM_PLUGIN_URL . 'assets/img/FB_Wireframe.png' ?>" alt="filebird">
                </div>
            </div>
            <style>
                .rjte-d-row{-webkit-box-align:center;-ms-flex-align:center;align-items:center;display:-webkit-box;display:-ms-flexbox;display:flex}.rjte-justify-between{-webkit-box-pack:justify;-ms-flex-pack:justify;justify-content:space-between}.rjte-ads-info{padding:15px 0 30px}@media screen and (max-width:782px){.rjte-ads-info{padding:0}}.rjte-ads-info p{margin-bottom:25px}.rjte-ads-title{font-size:16px;margin:0 0 15px}.rjte-ads-img{max-width:252px}.rjte-btn-row{line-height:30px}.rjte-btn-row>.button{margin-right:10px}
            </style>
            <?php
        }
        ...
        public function add_dashboard_widget(){
            ?>
            <style>
                #dashboard-widgets .rjte-postbox-title-wrap {
                    margin: 15px 0;
                }
                #dashboard-widgets .rjte-postbox-title-wrap>h3 {
                    font-size: 14px;
                    font-weight: 600;
                    padding: 0;
                    margin: 0 0 10px;
                ...
                #dashboard-widgets .rjte-postbox-title-wrap>span {
                        font-size: 14px;
                opacity: .9;
                    }
            ...
            <div class="rjte-wrap-postbox">
              <div class="rjte-postbox-title-wrap">
        ...
        public function ajax_install_plugin(){
            check_ajax_referer("rjte_filebird_cross_nonce", 'nonce', true);
        ...
        public function ajax_set_notification(){
            check_ajax_referer("rjte_{$this->pluginPrefix}_cross_nonce", 'nonce', true);
            //Save after 30 days
            update_option("rjte_notification_{$this->pluginPrefix}_cross", time() + (30 * 60 * 60 * 24));
            wp_send_json_success();
        }
        ...
        public function ajax_hide_cross(){
            ...
            update_option("rjte_{$type}_{$this->pluginPrefix}_cross", $time);
            wp_send_json_success();
        }
    }
}
...
if ( !class_exists('FileBirdCross') ) {
    class FileBirdCross extends rjteCross {
        public function is_plugin_exist()
        {
            return ( 
                defined('RJ_FILEBIRD_VERSION') ||
    ...
    FileBirdCross::get_instance('filebird', 'filebird+ninjateam', RJ_TELEGRAM_PLUGIN_URL, array('filebird/filebird.php', 'filebird-pro/filebird.php'));
}
