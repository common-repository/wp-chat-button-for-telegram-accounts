    ...
    class rjteReview
    {
        ...
        public function add_notification() {
            if (function_exists('get_current_screen')) {
                if (get_current_screen()->id == 'plugins' || get_post_type() == 'telegram-accounts' ) {
                    ...
                    <script>
                    jQuery("#rj_te-review a").on("click", function () {
                        var thisElement = this;
                        var fieldValue = jQuery(thisElement).attr("data");
                        var link = "https://codecanyon.net/item/telegram-chat-wordpress/reviews/22800580";
                        var hidePopup = false;
                        ...
                        jQuery.ajax({
                            ...
                            type: "post",
                            data: {
                                action: 'rj_te_save_review',
                                field: fieldValue,
                                nonce: '<?php echo esc_attr(wp_create_nonce("rjte_filebird_cross_nonce")) ?>',
                            },
                            }).done(function (result) {
                            if (result.success) {
                                if (hidePopup == true) {
                                    jQuery('#rj_te-review').hide("slow");
                                }
                            } else {
...
if ( !class_exists('RJTelegramReview') ) {
    class RJTelegramReview extends rjteReview {}
    RJTelegramReview::get_instance('rj_te', 'Telegram Plugin', 'rj-telegram');
}
