...
function rj_block_cgb_block_assets()
{ // phpcs:ignore
	...
	wp_localize_script(
		'block-cgb-block-js',
		'rjtewa', // Array containing dynamic data for a JS Global.
		[
			'pluginDirPath' 	=> plugin_dir_path(__DIR__),
			'pluginDirUrl'  	=> plugin_dir_url(__DIR__),
			'avatarDefaultUrl' 	=> RJ_TELEGRAM_PLUGIN_URL . 'assets/img/telegram_logo.svg',
			'gutenbergPreview'  => RJ_TELEGRAM_PLUGIN_URL . 'assets/img/telegram-button-preview.png',
			'nonce'				=> wp_create_nonce('rjte-wa-gutenberg')
	...
	register_block_type(
		...
		array(
			...
			'render_callback' => 'rj_te_button_render',
			'attributes' => array(
...
function rj_te_button_render($attributes)
{
	if ($attributes['isSelectedAccount'] != -1) {
        return do_shortcode("[rj_button id={$attributes['isSelectedAccount']}]");
    } else {
        ...
        if (empty($attributes['imageUrl'])) {
            $btn_icon_or_image = '<div class="wa__btn_icon"><img src="' . RJ_TELEGRAM_PLUGIN_URL . 'assets/img/telegram_logo.svg' . '" alt=' . $attributes['imageAlt'] . '/></div>';
        } else {
        ...
        $html .= '<a target="_blank" href="https://telegram.me/' . $attributes['phoneNumber'] . '" class="wa__button ' . $btnStyleClass . ' wa__stt_online ' . $avatarClass . '" style="background-color: ' . $attributes['buttonColor'] . '; color: ' . $attributes['textColor'] . '">';
...
function rj_assets()
{
	wp_enqueue_style('nta-css-popup', RJ_TELEGRAM_PLUGIN_URL . 'assets/css/style.css');
}

// Hook: Block assets.
add_action('init', 'rj_block_cgb_block_assets');
add_action('enqueue_block_assets', 'rj_assets');
